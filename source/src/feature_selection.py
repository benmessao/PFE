...
X = abs(
    data.iloc[:, 0:83]
)  # On prend la valeur absolue car les valeurs doivent être positives pour utiliser la méthode Chi2
y = data.iloc[:, -1]
...
def selection_RF(X, y):
    ...
    embeded_rf_support = [
        True if i in feat_importances.nlargest(20).index else False
...
def selection_recurcive_feature(X, y):
    rfe_selector = RFE(
        estimator=LogisticRegression(), n_features_to_select=20, step=10, verbose=5
    )
