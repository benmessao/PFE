...
def CNN_LSTM_2(input_sequence_shape):
    ...
    model.summary()
    model.compile(loss='mae', optimizer=adam, metrics=['accuracy', keras.metrics.Precision(), keras.metrics.Recall(), F1Score])

    return model

def three_LSTM(input_sequence_shape):
    model = keras.Sequential()
    model.add(keras.layers.LSTM(256, return_sequences=True, activation='relu', input_shape=input_sequence_shape))
    model.add(keras.layers.LSTM(256, return_sequences=True, activation='relu'))
    model.add(keras.layers.LSTM(256, return_sequences=False, activation='relu'))
    model.add(keras.layers.Dense(1, activation='relu'))
...
def four_LSTM(input_sequence_shape):
    ...
    adam = keras.optimizers.Adam(learning_rate=0.0003)
    model.compile(loss='mean_absolute_error', optimizer=adam, metrics=['accuracy', keras.metrics.Precision(), keras.metrics.Recall(), F1Score])
...
def LSTM_1(input_sequence_shape):
    model = keras.Sequential()
    model.add(keras.layers.LSTM(512, return_sequences=True, activation='relu', input_shape=input_sequence_shape))
